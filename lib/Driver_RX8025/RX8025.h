...
//    RX8025(I2C& p_i2c);
    ...
    /** Read one byte from specific register
      ...
      * @param pdata Dato le闊塷 del registro
      * @return C楠磇go de error <0
      */
...
private:
     ...
     * @return C楠磇go de error (-1) o 0
     */
    int8_t set_alarmD_reg (uint16_t time);


    /** Set Alarm-D / INTA time
      * @param next time (unit: minutes) from now on minimum = 2 minute閿濓拷!!
